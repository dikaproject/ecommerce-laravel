...
@section('title', 'Tambah Diskon - Admin DesignArt Studio')

@section('content')
<div class="p-6">
    <div class="mb-6">
        <h1 class="text-2xl font-semibold text-gray-800">Tambah Diskon</h1>
        <p class="text-gray-600">Tambahkan kode diskon baru</p>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('admin.discounts.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="code" id="code" value="{{ old('code') }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-indigo-500 @error('code') border-red-500 @enderror" required>
                @error('code')
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="3" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-indigo-500 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="value" id="value" value="{{ old('value') }}" step="0.01" min="0" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-indigo-500 @error('value') border-red-500 @enderror" required>
                @error('value')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4" id="max-discount-field">
                ...
                <input type="number" name="max_discount" id="max_discount" value="{{ old('max_discount') }}" step="0.01" min="0" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-indigo-500 @error('max_discount') border-red-500 @enderror">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="datetime-local" name="valid_from" id="valid_from" value="{{ old('valid_from') }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-indigo-500 @error('valid_from') border-red-500 @enderror" required>
                    @error('valid_from')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="datetime-local" name="valid_until" id="valid_until" value="{{ old('valid_until') }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-indigo-500 @error('valid_until') border-red-500 @enderror" required>
                    @error('valid_until')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
